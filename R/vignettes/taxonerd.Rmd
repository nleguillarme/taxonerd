...
This will install the last release of TaxoNERD available on PyPI in a virtual environment named r-taxonerd. If you want TaxoNERD to run on a GPU, you will have to install the package with GPU support. First, find your CUDA version :
...
In this example, we will use the en_ner_eco_md model with the abbreviation detector enabled, and we will ask the engine to link detected taxon mentions to a subset of the NCBI Taxonomy. Entity linking relies on a approximate nearest neighbors search and uses a minimum similarity threshold to filter out entities whose name is not similar enough to the textual mention. Here, we will set the minimum similarity threshold to 0.85.

```{r}
# Initialize the taxonomic NER engine
ner <- init.taxonerd("en_ner_eco_md", abbrev=TRUE, sent=FALSE, link="ncbi_lite", thresh=0.85, gpu=FALSE)
```
...
list.of.dfs <- find.in.corpus(ner, corpus.dir)
```
